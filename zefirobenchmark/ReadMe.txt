...
■アプリケーションの概要
800x600のスクリーンでの[fps]、[texel/s]、[tri/s]を測定してファイルに出力します。


■ベンチマーク
　ベンチマークは、メニュから実行することができます。
　ベンチマークは、ESCキーで中断することができます。
　ウィンドウモード／フルスクリーンモードに対応し、
　不透明テクスチャと半透明テクスチャの場合の測定が可能です。

  描画するテクスチャサイズは16x16、32x32、64x64、128x128、256x256の正方テクスチャである。

　不透明では、テクスチャ数1000,2500,5000,10000,20000,40000で測定します。
　不透明の場合にはカレントディレクトリの16.png、32.png、64.png、128.png、256.pngを対応するテクスチャとして読み込む。
　半透明では、テクスチャ数100,250,500,1000,2000,4000で測定します。
　半透明の場合にはカレントディレクトリの16a.png、32a.png、64a.png、128a.png、256a.pngを対応するテクスチャとして読み込む。

　結果はzefirobenchmarkoutput.txtに出力されます。出力結果はファイルの最後尾に付け足されますので、
　いらない場合は以前のファイルを消去してください。

■注意
・ベンチマークは途中で終了することができません。
・テクスチャファイルがない場合・DirectX8.1がインストールしてない場合・デバイスが描画に対応してない場合の
動作は保障しておりません。気をつけてご利用ください。


■バージョン履歴
1.1 ESCキーでベンチマークの途中で処理を抜けられるようにした。
